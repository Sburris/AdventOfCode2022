...
func main () {
	...
	currentSize := fs.RootFolder.GetSize()
	availableSpace := 70000000 - currentSize
	minimumNeededSpace := 30000000 - availableSpace

	potentialDeletions := make([]int, 0)
	for _, size := range dirList {
		if size >= minimumNeededSpace {
			potentialDeletions = append(potentialDeletions, size)
		}
	}

	smallest := math.MaxInt32
	for _, size := range potentialDeletions {
		if size < smallest {
			smallest = size
		}
	}
